...
public class IntelligenceCommand {

    @SubscribeEvent
    public static void onCommand(RegisterCommandsEvent event) {
        CommandDispatcher<CommandSourceStack> dispatcher = event.getDispatcher();

        dispatcher.register(Commands.literal("intelligence")
                .then(Commands.literal("spend")
                        .then(Commands.argument("value", IntegerArgumentType.integer(1))
                        .executes(IntelligenceCommand::spendPoints))));
        ...
        try {
            ...
            player.getCapability(PlayerAttributesProvider.PLAYER_ATTRIBUTES).ifPresent(attributes -> {
                if (attributes.getAttributePoints() >= value) {
                    attributes.spendAttributePoints(Attributes.INTELLIGENCE, value);
                    player.sendSystemMessage(Component.literal("You spent " + value + " points on Intelligence!"));
