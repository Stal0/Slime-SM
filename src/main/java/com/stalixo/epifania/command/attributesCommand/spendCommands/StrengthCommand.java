    ...
    public static void onCommand(RegisterCommandsEvent event) {
        ...
        dispatcher.register(Commands.literal("strength")
                .then(Commands.literal("spend")
                        ...
                        .executes(StrengthCommand::spendPoints))));
        ...
        try {
            ...
            player.getCapability(PlayerAttributesProvider.PLAYER_ATTRIBUTES).ifPresent(attributes -> {
                if (attributes.getAttributePoints() >= value) {
                    attributes.spendAttributePoints(Attributes.STRENGTH, value);
                    player.sendSystemMessage(Component.literal("You spent " + value + " points on Strength!"));
